...
/**
 * 文件级说明：测试 TestFlinkConfParser 的功能。
 *
 * <p>所属模块：iceberg-flink（flink v1.15）。职责：验证 TestFlinkConfParser 在各类场景下的行为是否符合预期， 包括正常路径与边界条件。
 *
...
public class TestFlinkConfParser {
   ...
   * 测试场景：Duration Conf。
   *
   * <p>验证该方法在 Duration Conf 条件下的行为是否符合预期。
   */
  @Test
