 ...
 *
 * <p>职责：Iceberg 表的 Spark 包装，实现 Spark V2 Table 接口，支持读写、行级操作、元数据列与时间旅行。
 *
 * <p>设计意图：委托底层 Iceberg Table，将 Spark 的读写/扫描/写入请求转译为 Iceberg 操作，并通过快照后缀支持时间旅行。
 *
 * <p>上下游关系：由 SparkCatalog / IcebergSource 创建；被 Spark 读写链路调用。
 */
