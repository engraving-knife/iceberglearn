 ...
 *
 * <p>职责：添加文件的存储过程，将外部数据文件导入到指定 Iceberg 表分区。
 *
 * <p>设计意图：通过 SparkTableUtil 列举并注册外部文件，支持指定分区与文件格式。
 *
