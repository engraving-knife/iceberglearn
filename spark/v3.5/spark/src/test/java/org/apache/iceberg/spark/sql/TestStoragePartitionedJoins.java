 ...
 *
 * <p>所属模块：iceberg-spark（spark v3.5）。职责：验证 Iceberg 表在 Spark 引擎下 storage分区连接 相关行为，覆盖正常路径与边界场景。
 *
...
public class TestStoragePartitionedJoins extends SparkTestBaseWithCatalog {
  ...
  /** 测试连接带bucketing上时间戳ntz列场景：验证该方法在对应输入下的行为与断言结果。 */
  @Test
  ...
  /** 测试连接带years上时间戳ntz列场景：验证该方法在对应输入下的行为与断言结果。 */
  @Test
  ...
  /** 测试连接带months上时间戳ntz列场景：验证该方法在对应输入下的行为与断言结果。 */
  @Test
  ...
  /** 测试连接带days上时间戳ntz列场景：验证该方法在对应输入下的行为与断言结果。 */
  @Test
  ...
  /** 测试连接带hours上时间戳ntz列场景：验证该方法在对应输入下的行为与断言结果。 */
  @Test
  ...
  /** 测试连接带mismatching分区keys场景：验证该方法在对应输入下的行为与断言结果。 */
  @Test
