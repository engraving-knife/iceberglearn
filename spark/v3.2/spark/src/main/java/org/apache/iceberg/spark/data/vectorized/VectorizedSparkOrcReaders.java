 ...
 *
 * <p>所属模块：iceberg-spark v3.2。 类型：类 VectorizedSparkOrcReaders。
 *
...
public class VectorizedSparkOrcReaders {
   ...
   *
   * <p>所属模块：iceberg-spark v3.2。 类型：接口 Converter。
   *
   ...
   *
   * <p>所属模块：iceberg-spark v3.2。 类型：类 BaseOrcColumnVector。
   *
   ...
   * Spark 向量化读取 Iceberg 数据的列式访问组件，封装列向量相关能力。
   *
   * <p>所属模块：iceberg-spark v3.2。 类型：类 PrimitiveOrcColumnVector。
   *
   * <p>上下游：被 SparkScan/SparkWrite 调用，依赖 Iceberg 文件格式读取/写入 API。
   */
   ...
   * Spark 向量化读取 Iceberg 数据的列式访问组件，负责类型或表达式转换。
   *
   * <p>所属模块：iceberg-spark v3.2。 类型：类 ArrayConverter。
   *
   * <p>设计意图：适配器模式，桥接两套 API。
   *
   * <p>上下游：被 SparkScan/SparkWrite 调用，依赖 Iceberg 文件格式读取/写入 API。
   ...
   * Spark 向量化读取 Iceberg 数据的列式访问组件，负责类型或表达式转换。
   *
   * <p>所属模块：iceberg-spark v3.2。 类型：类 MapConverter。
   *
   * <p>设计意图：适配器模式，桥接两套 API。
   *
   * <p>上下游：被 SparkScan/SparkWrite 调用，依赖 Iceberg 文件格式读取/写入 API。
   ...
   * Spark 向量化读取 Iceberg 数据的列式访问组件，负责类型或表达式转换。
   *
   * <p>所属模块：iceberg-spark v3.2。 类型：类 StructConverter。
   *
   * <p>设计意图：适配器模式，桥接两套 API。
   *
   * <p>上下游：被 SparkScan/SparkWrite 调用，依赖 Iceberg 文件格式读取/写入 API。
