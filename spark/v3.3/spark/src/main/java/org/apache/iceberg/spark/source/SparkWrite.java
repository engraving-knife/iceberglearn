 ...
 *
 * <p>所属模块：iceberg-spark v3.3。 类型：类 SparkWrite。
 *
...
abstract class SparkWrite implements Write, RequiresDistributionAndOrdering {
   ...
   * Iceberg 表在 Spark DataSource V2 中的实现的写入组件，负责数据写入与提交。
   *
   * <p>所属模块：iceberg-spark v3.3。 类型：类 BaseBatchWrite。
   *
   ...
   *
   * <p>所属模块：iceberg-spark v3.3。 类型：类 BatchAppend。
   *
   * <p>上下游：被 SparkCatalog 创建，依赖 Iceberg Table API 与底层扫描/写入组件。
   */
   ...
   * Iceberg 表在 Spark DataSource V2 中的实现的写入组件，负责数据写入与提交。
   *
   * <p>所属模块：iceberg-spark v3.3。 类型：类 DynamicOverwrite。
   *
   * <p>上下游：被 SparkCatalog 创建，依赖 Iceberg Table API 与底层扫描/写入组件。
   */
   ...
   * Iceberg 表在 Spark DataSource V2 中的实现的写入组件，负责数据写入与提交。
   *
   * <p>所属模块：iceberg-spark v3.3。 类型：类 OverwriteByFilter。
   *
   * <p>上下游：被 SparkCatalog 创建，依赖 Iceberg Table API 与底层扫描/写入组件。
   */
   ...
   * Iceberg 表在 Spark DataSource V2 中的实现的写入组件，负责数据写入与提交。
   *
   * <p>所属模块：iceberg-spark v3.3。 类型：类 CopyOnWriteOperation。
   *
   * <p>上下游：被 SparkCatalog 创建，依赖 Iceberg Table API 与底层扫描/写入组件。
   */
   ...
   * Iceberg 表在 Spark DataSource V2 中的实现的写入组件，负责数据写入与提交。
   *
   * <p>所属模块：iceberg-spark v3.3。 类型：类 RewriteFiles。
   *
   ...
   * Iceberg 表在 Spark DataSource V2 中的实现的写入组件，负责数据写入与提交。
   *
   * <p>所属模块：iceberg-spark v3.3。 类型：类 BaseStreamingWrite。
   *
   * <p>设计意图：模板方法模式，抽取公共流程供子类复用。
   *
   * <p>上下游：被 SparkCatalog 创建，依赖 Iceberg Table API 与底层扫描/写入组件。
   ...
   *
   * <p>所属模块：iceberg-spark v3.3。 类型：类 StreamingAppend。
   *
   * <p>上下游：被 SparkCatalog 创建，依赖 Iceberg Table API 与底层扫描/写入组件。
   */
   ...
   * Iceberg 表在 Spark DataSource V2 中的实现的写入组件，负责数据写入与提交。
   *
   * <p>所属模块：iceberg-spark v3.3。 类型：类 StreamingOverwrite。
   *
   * <p>上下游：被 SparkCatalog 创建，依赖 Iceberg Table API 与底层扫描/写入组件。
   */
   ...
   * Iceberg 表在 Spark DataSource V2 中的实现。
   *
   * <p>所属模块：iceberg-spark v3.3。 类型：类 TaskCommit。
   *
   * <p>上下游：被 SparkCatalog 创建，依赖 Iceberg Table API 与底层扫描/写入组件。
   */
   ...
   *
   * <p>所属模块：iceberg-spark v3.3。 类型：类 WriterFactory。
   *
   ...
   *
   * <p>所属模块：iceberg-spark v3.3。 类型：类 UnpartitionedDataWriter。
   *
   * <p>上下游：被 SparkCatalog 创建，依赖 Iceberg Table API 与底层扫描/写入组件。
   */
   ...
   * Iceberg 表在 Spark DataSource V2 中的实现的写入器，负责把 Spark 内部数据写入 Iceberg 底层存储。
   *
   * <p>所属模块：iceberg-spark v3.3。 类型：类 PartitionedDataWriter。
   *
   * <p>上下游：被 SparkCatalog 创建，依赖 Iceberg Table API 与底层扫描/写入组件。
   */
