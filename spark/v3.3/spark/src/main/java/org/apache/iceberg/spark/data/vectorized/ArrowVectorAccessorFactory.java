 ...
 *
 * <p>所属模块：iceberg-spark v3.3。 类型：类 ArrowVectorAccessorFactory。
 *
...
final class ArrowVectorAccessorFactory
   ...
   * Spark 向量化读取 Iceberg 数据的列式访问组件的工厂，负责创建实例。
   *
   * <p>所属模块：iceberg-spark v3.3。 类型：类 DecimalFactoryImpl。
   *
   * <p>设计意图：工厂模式，集中创建逻辑便于扩展。
   *
   * <p>上下游：被 SparkScan/SparkWrite 调用，依赖 Iceberg 文件格式读取/写入 API。
   ...
   * Spark 向量化读取 Iceberg 数据的列式访问组件的工厂，负责创建实例。
   *
   * <p>所属模块：iceberg-spark v3.3。 类型：类 StringFactoryImpl。
   *
   * <p>设计意图：工厂模式，集中创建逻辑便于扩展。
   *
   * <p>上下游：被 SparkScan/SparkWrite 调用，依赖 Iceberg 文件格式读取/写入 API。
   ...
   * Spark 向量化读取 Iceberg 数据的列式访问组件的工厂，负责创建实例。
   *
   * <p>所属模块：iceberg-spark v3.3。 类型：类 ArrayFactoryImpl。
   *
   * <p>设计意图：工厂模式，集中创建逻辑便于扩展。
   *
   * <p>上下游：被 SparkScan/SparkWrite 调用，依赖 Iceberg 文件格式读取/写入 API。
   ...
   * Spark 向量化读取 Iceberg 数据的列式访问组件的工厂，负责创建实例。
   *
   * <p>所属模块：iceberg-spark v3.3。 类型：类 StructChildFactoryImpl。
   *
   * <p>设计意图：工厂模式，集中创建逻辑便于扩展。
   *
   * <p>上下游：被 SparkScan/SparkWrite 调用，依赖 Iceberg 文件格式读取/写入 API。
