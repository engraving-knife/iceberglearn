...
/**
 * 测试类：TestSequenceNumberForV2Table，用于验证 Sequence Number For 2 Table 相关功能。
 *
 * <p>所属模块：iceberg-core（测试目录 src/test）。 职责：针对 Sequence Number For 2 Table
...
public class TestSequenceNumberForV2Table extends TableTestBase {
  ...
  /** 辅助方法：sequence number for 2 table。 */
  ...
  /**
   * 测试场景：multiple operations transaction。
   *
   * <p>验证逻辑：针对该场景调用被测方法，断言返回结果或表/快照状态符合预期。
   */
  @Test
  ...
  /**
   * 测试场景：expiration in transaction。
   *
   * <p>验证逻辑：针对该场景调用被测方法，断言返回结果或表/快照状态符合预期。
   */
  @Test
