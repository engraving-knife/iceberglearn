...
/**
 * 测试类：TestScanSummary，用于验证 Scan Summary 相关功能。
 *
 * <p>所属模块：iceberg-core（测试目录 src/test）。 职责：针对 Scan Summary 的核心行为构造多种场景，覆盖正常路径、边界条件与异常输入，
 * 确保实现与预期语义一致。
 *
...
public class TestScanSummary extends TableTestBase {
  ...
  /**
   * 测试场景：snapshot time range validation。
   *
