...
/**
 * 测试类：TestSnapshot，用于验证 Snapshot 相关功能。
 *
 * <p>所属模块：iceberg-core（测试目录 src/test）。 职责：针对 Snapshot 的核心行为构造多种场景，覆盖正常路径、边界条件与异常输入， 确保实现与预期语义一致。
 *
...
public class TestSnapshot extends TableTestBase {
  ...
  /**
   * 测试场景：sequence numbers in added data files。
   *
   * <p>验证逻辑：针对该场景调用被测方法，断言返回结果或表/快照状态符合预期。
   */
  @Test
  ...
  /** 辅助方法：run added data file sequence number test。 */
  ...
  /**
   * 测试场景：sequence numbers in removed data files。
   *
   * <p>验证逻辑：针对该场景调用被测方法，断言返回结果或表/快照状态符合预期。
   */
  @Test
  ...
  /** 辅助方法：run removed data file sequence number test。 */
  ...
  /**
   * 测试场景：sequence numbers in added delete files。
   *
   * <p>验证逻辑：针对该场景调用被测方法，断言返回结果或表/快照状态符合预期。
   */
  @Test
  ...
  /** 辅助方法：run added delete file sequence number test。 */
