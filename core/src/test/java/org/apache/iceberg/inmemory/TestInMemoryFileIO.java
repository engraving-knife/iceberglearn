...
/**
 * 测试类：TestInMemoryFileIO，用于验证 In Memory File IO 相关功能。
 *
 * <p>所属模块：iceberg-core（测试目录 src/test）。 职责：针对 In Memory File IO 的核心行为构造多种场景，覆盖正常路径、边界条件与异常输入，
 * 确保实现与预期语义一致。
 *
...
public class TestInMemoryFileIO {
  ...
  /**
   * 测试场景：overwrite before and after close。
   *
   * <p>验证逻辑：针对该场景调用被测方法，断言返回结果或表/快照状态符合预期。
   */
  @Test
